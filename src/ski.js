...
function makeChunk(i) {
  ...
  const corridor = Math.max(3, 9.5 - i * 1.3);
  const nTrees = Math.min(150, 40 + i * 6);
  let tries = 0;
  while (c.trees.length < nTrees && tries++ < nTrees * 12) {
    const x = rr(-(T.halfWidth + 14), T.halfWidth + 14), z = z0 + rr(0, L);
    const localCorridor = i === 0 ? lerp(30, corridor, clamp((z - z0 - 6) / (L - 6), 0, 1)) : corridor;
    if (Math.abs(x) < localCorridor && i < 4) continue;
    if (Math.abs(x) < 5 && i < 8 && rng() < 0.7) continue;
...
function updateScarf(t, speed, air) {
  ...
  for (let i = 0; i <= segs; i++) {
    ...
    const x = scarfBase.x + wave, y = scarfBase.y + 0.06 - drop, z = scarfBase.z - 0.05 - u * len;
    const k = i * 6;
...
window.__ski = { state, input: ctrl, T, step, syncVisuals, start: startRun, reset: resetRun, terrainH, chunks, scene, engine, sim: SIM };
